...
#variable initialization regarding helium
N = 200 #number of helium atoms need to render
N_real = 6E23  #total number of helium atoms / one mole
m = 4E-3*(N/N_real)
size = 31E-12*10
L = ((10E-3)/(2*pi))**(1/3) + size 
k, T = 1.38E-23, 298
...
bullet = sphere(pos=vector(2*L,0,0),radius=L, opacity=0.2, color = color.red )
...
for i in range(N):
    p_a[i] = [L*random() , 0,0] # particle is initially random positioned in container 
...
while Nstop:
    ...
    if stage<=2:
        for i in range(N):
            ...
            if p_a[i][0] <= container.pos.x + size and p_a[i][0]*v_a[i][0] > 0 :
                v_a[i][0] = - (abs(v_a[i][0])+2*v_W)*v_a[i][0]/abs(v_a[i][0])
                dP+=2*m*(abs(v_a[i][0]+2*abs(v_W)))
    ...
    if times%1000==0:
        ...
        print('T=',T)
        print('dP=',dP)
        p=dP/(1000*dt)/(2*pi*L**2+2*pi*L*container.length)
        V=pi*L**2*container.length
